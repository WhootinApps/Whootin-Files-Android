package com.whootin.whootinviewpager;
...
public class AniViewPager extends ViewPager {
	...
	public AniViewPager(Context context) {
		this(context, null);
	}

	@SuppressWarnings("incomplete-switch")
	public AniViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		setClipChildren(false);
		// now style everything!
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AniViewPager);
		int effect = ta.getInt(R.styleable.AniViewPager_style, 5);
		String[] transitions = getResources().getStringArray(R.array.animation_effects);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(R.styleable.AniViewPager_fadeEnabled, false));
		setOutlineEnabled(ta.getBoolean(R.styleable.AniViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.AniViewPager_outlineColor, Color.WHITE));
		switch (mEffect) {
		case Stack:
		case ZoomOut:
			setFadeEnabled(true);
